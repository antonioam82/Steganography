...
class app():
    def __init__(self):
        ...
        self.mode.set("EN")
        self.current_marker = tk.StringVar()
        self.current_marker.set("=====")
        self.spaces = 5
        self.nbytes = tk.IntVar()
        self.file_name = ""
        self.running = False
        self.interrupt = False
 
        self.entryDir = tk.Entry(self.window,width=117,textvariable=self.current_dir)
        self.entryDir.place(x=0,y=0)
        self.textEntry = sct.ScrolledText(self.window,width=85,height=16)
        self.textEntry.place(x=5,y=28)
        tk.Button(self.window,text="CLEAR TEXT",bg=self.backgr,width=20,command=self.clear).place(x=5,y=311)
        self.rdbEncode = tk.Radiobutton(self.window,text="Encode",variable=self.mode,value="EN",command=self.set_mode)
        self.rdbEncode.place(x=553,y=313)
        self.rdbDecode = tk.Radiobutton(self.window,text="Decode",variable=self.mode,value="DE",command=self.set_mode)
        self.rdbDecode.place(x=629,y=313)
        tk.Button(self.window,text="SEARCH",width=20,bg=self.backgr,command=self.open_file).place(x=5,y=345)
        self.entImage = tk.Entry(self.window,width=49,font=('arial',14),textvariable=self.imaname)
        self.entImage.place(x=156,y=345)
        self.btnStart = tk.Button(self.window,text="START ENCODING",width=76,bg=self.backgr,command=self.init_task)
        self.btnStart.place(x=158,y=379)
        tk.Label(self.window,text="BYTES AVAILABLE:").place(x=167,y=314)
        self.byEnt = tk.Entry(self.window,textvariable=self.nbytes,width=11)
        self.byEnt.place(x=271,y=315)
        tk.Label(self.window,text="MARKER:").place(x=354,y=314)
        self.markerEntry = tk.Entry(self.window,textvariable=self.current_marker)
        self.markerEntry.place(x=410,y=315)
        self.invLabel = tk.Label(self.window,fg="blue",width=98)
        self.invLabel.place(x=6,y=289)
        self.btn_copim = tk.Button(self.window,text="IMPORT DATA",bg=self.backgr,width=20,command=self.init_copy)
    ...
    def copytext(self):
        try:
            if len(self.textEntry.get('1.0',tk.END)) > 1:
                pyperclip.copy(self.textEntry.get('1.0',tk.END))
    ...
    def copy_text(self):
        ...
        while True:
            ...
            if self.copia != self.ultima_copia:
                self.textEntry.insert(tk.END,self.copia)
...
select 'copy' to import it.""")
    ...
    def encode(self):
        secret_data = self.textEntry.get('1.0',tk.END)
    ...
    def init_task(self):
        if self.current_marker.get() != "":
            if self.file_name != "":
                if self.mode.get()=="EN" and len(self.textEntry.get('1.0',tk.END))>1 and self.running==False:
                    self.running = True
